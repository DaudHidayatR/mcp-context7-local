...
async function safeReadJsonFile(filePath: string): Promise<unknown | null> {
  try {
...
export async function loadProjectContext(
  ...
  for (const section of sections) {
    ...
    const value = await safeReadJsonFile(filePath);
    if (value !== null) {
...
export async function handleGetProjectContext(
  ...
  const context = await loadProjectContext(prdDir, task_type, namespace);
